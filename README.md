...
To update the database use this command from the Web project folder (replace `API_CleanArchitecture` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../API_CleanArchitecture.Infrastructure/API_CleanArchitecture.Infrastructure.csproj -s API_CleanArchitecture.Web.csproj
```
